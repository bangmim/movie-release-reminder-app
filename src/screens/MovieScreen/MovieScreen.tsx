...
const styles = StyleSheet.create({
  ...
  infoTexts: { flex: 1, marginLeft: 20 },
  titleTexts: {
    flex: 1,
  },
  titleText: {
  ...
  originalTitle: { marginTop: 2, fontSize: 14, color: Colors.white },
  releaseDateText: { marginTop: 4, fontSize: 16, color: Colors.white },
  overviewText: { fontSize: 14, color: Colors.white },
  separator: {
    width: 16,
  },
  verticalSeparator: { height: 16 },
  addToCalendarButton: {
    marginTop: 8,
  ...
  addToCalendarButtonText: { color: Colors.black },
});
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {hasReminder(`${movie.id}`) ? (
          ...
          <TouchableOpacity
            onPress={async () => {
              try {
                await addReminder(releaseDate, title, movie.id);
                Alert.alert('알림이 등록되었습니다');
              } catch (error: any) {
                Alert.alert(error.message);
              }
            }}
